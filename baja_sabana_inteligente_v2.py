...
directorio = 'S:\\Finanzas\\Trading\\Quant\\'
...
parsed_lista_sabanas_dia_all_characters=[]
  
if  not not lista_sabanas_dia:
    for gg in lista_sabanas_dia:
        parse_text = gg.split("_")
        parsed_lista_sabanas_dia.append(parse_text[4])
        parsed_lista_sabanas_dia_all_characters.append(parse_text)
    #Recuperando los ID como values
    ID_list=[int(x) for x in parsed_lista_sabanas_dia]
    ID_maximo=max(ID_list)
    
    #Eligiendo la base mas actual en la lista (la con mayor ID)
    #Ahora necesito recuperar el string completo de la data mas actual
    #Como ya conozco el ID, puedo identificarla en
    #la lits "parsed_lista_sabanas_dia_all_characters" y de aca recuperar la 
    #fecha
    SABANA_DIA_ID_MAX = []
    for qq in parsed_lista_sabanas_dia_all_characters:
        if qq[4]==str(ID_maximo):
            fecha_max_id = qq[2]
            
            
            
    SABANA_DIA_ID_MAX = 'SABANA_DIA_'+fecha_max_id+'_ID_'+str(ID_maximo)+'_.txt'
...
if  not not lista_sabanas_dia:
    ...
    star_old= datetime.now().strftime("%m/%d/%Y") 
    ...
    fecha_ticker_old = tickers_old.index[-1].strftime("%m/%d/%Y")
    fecha_base= base.index[-1].strftime("%m/%d/%Y")
    if fecha_ticker_old ==fecha_base :
        base = base.iloc[:-1,:]
        tickers_update=base.append(tickers_old)
    else:       
        tickers_update=base.append(tickers_old)    
    ...
    if  not not lista_sabanas_dia:
        ...
        hora=hora.replace(" ","_")
        hora=hora.replace(":","_")
        tickers_update.to_csv(directorio+'SABANA_DIA'+hora+'_ID_'+'_.txt', sep='\t')
        ID_maximo_new = ID_maximo +1
        SendSabana(directorio+'SABANA_DIA'+hora+'_ID_'+ID_maximo_new+'_.txt')
        ...
        hora=hora.replace(" ","_")
        hora=hora.replace(":","_")
        tickers_update.to_csv(directorio+'SABANA_DIA'+hora+'_ID_1'+'_.txt', sep='\t')
        SendSabana(directorio+'SABANA_DIA'+hora+'_ID_1'+'_.txt')            
    ...
    hora=hora.replace(" ","_")
    hora=hora.replace(":","_")
    sabana_1st.to_csv(directorio+'SABANA_FUENTE'+hora+'.txt', sep='\t') 
    SendSabana(directorio+'SABANA_FUENTE'+hora+'.txt')     
